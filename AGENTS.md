...
- Production resource names are `mpp-observatory` (D1), `mpp-observations` (R2), `mpp-crawl` (Queue), and `mpp-crawl-dlq` (dead-letter Queue). All four are provisioned, migration `0001_observatory.sql` is applied to remote D1, and the enabled `observations-30d` lifecycle must continue expiring `observations/` objects after 30 days.
